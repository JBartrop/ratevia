...
const Account : React.FC = () => {
    ...
    const [Users, setUsers] = useState <UserRow[]>(listuser)
    ...
    const editrow = (values: UserRow) => {
        setEditingRow(values)
    };

    const deleterow = (values: UserRow) => {
         setDeletingRow(values)
    };

    const DeleteData= (values: UserRow) => {
    ...
    return(
        <section className="w-full bg-[rgb(var(--card))] p-4 rounded-md">
            <div className="mb-4 flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-[rgb(var(--text))]" >All User Management</h1>
                </div>
                <div className="flex">
                    {/* anythig  */}
                </div>
            ...
            {/* delete  */}
            {deletingRow && (
              <div onClick={() => setDeletingRow(null)} className="fixed inset-0  bg-black/50 flex justify-center items-center z-50">
                <div onClick={(e) => e.stopPropagation()} className="bg-[rgb(var(--card))] p-6 rounded-lg w-5/6 sm:w-4/6 lg:w-1/2 shadow-lg">
                  <h2 className="text-lg font-bold mb-4 text-center">Are you sure you want to delete {deletingRow.name}</h2>
                  <div className="mt-6 flex justify-center gap-2">
                        <Button 
                            ...
                            size="sm" 
                            value="Delete" 
                            onClick={() => {DeleteData(deletingRow); setDeletingRow(null)}}
                            className= "bg-red-600 text-white hover:bg-red-600/80 px-3"
                        />
                        <Button 
                            type="button"
                            ...
                            className= "bg-gray-400 text-white hover:bg-gray-500/80 px-3"
                        />
